...
function ArchitecturalAccent({ position, rotation = [0.5, 0.5, 0.5], size = 2 }: {
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.5}>
      <mesh position={position} rotation={rotation}>
        ...
        <meshStandardMaterial color="#1A1A1A" wireframe transparent opacity={0.05} />
      </mesh>
    </Float>
  );
}
...
export default function ConstructionBackground() {
  return (
    <div className="fixed inset-0 -z-10 pointer-events-none overflow-hidden bg-[var(--color-background)] animate-in fade-in duration-1000">
